...
source <(curl -s https://raw.githubusercontent.com/FonicsScriptsDE/BashSelect.sh/refs/heads/main/BashSelect.sh)
clear
status(){
  clear
  echo -e $green$@'...'$reset
...
endpoint_add_tcp "0.0.0.0:30200"
endpoint_add_udp "0.0.0.0:30200"
...
if [[ $( echo $? ) == 0 ]]; then
  ...
  export OPTIONS=("Kill PID on port 40200" "Exit the script")
  bashSelect
  case $? in
    0 )
      status "killing PID on 40200"
      runCommand "apt -y install psmisc"
	  runCommand "fuser -4 40200/tcp -k"
      ;;
    1 )
      exit 0
      ;;
...
if [[ $phpmaInstall == 0 ]]; then
  bash <(curl -s https://raw.githubusercontent.com/FonicsScriptsDE/PHPMyAdminInstaller2025/refs/heads/main/install.sh) -s
fi
...
bold="\e[1m"
reset="\e[0m"
port=\$(lsof -Pi :40200 -sTCP:LISTEN -t)
if [ -z "\$port" ]; then
...
if [[ -z "$port" ]]; then
    ...
    txadmin="http://$(ifconfig eth0 | grep -Eo 'inet (addr:)?([0-9]*\.){3}[0-9]*' | grep -Eo '([0-9]*\.){3}[0-9]*' | grep -v '127.0.0.1'):40200"
